...
let canvas, ctx;
const W = 1000, H = 600;
const field = { left: 60, right: W - 60, top: 35, bottom: H - 35, goalTop: 215, goalBottom: 385, depth: 42 };
const localKeys = new Set(), remoteKeys = new Set();

let matchTimeLimit = 0, matchStartTime = 0, matchEnded = false, currentBallStyle = 0;
let mode = null, peer = null, connection = null, localChannel = null;
let scores = { green: 0, blue: 0 }, kickoffUntil = 0;
let lastTime = performance.now();
let lastStateSent = 0, lastInputSent = 0;
let botDifficulty = "beginner", botThinkAt = 0, botTarget = { x: 730, y: H / 2 };

const ball = { x: W / 2, y: H / 2, vx: 0, vy: 0, r: 13, mass: 0.75 };
const players = [
...
const botProfiles = {
  beginner: { speed: 190, reaction: 540, offset: 58, dribble: 10 },
  pro: { speed: 300, reaction: 230, offset: 46, dribble: 24 },
  goat: { speed: 410, reaction: 70, offset: 35, dribble: 42 }
};

function show(id) {
  ["menu", "lobby", "practice", "match"].forEach(x => {
    if ($(x)) $(x).classList.toggle("hidden", x !== id);
  });
}
...
function updateScore() {
  if ($("green-score")) $("green-score").textContent = scores.green;
  if ($("blue-score")) $("blue-score").textContent = scores.blue;
}

function resetPositions() {
  Object.assign(ball, { x: W / 2, y: H / 2, vx: 0, vy: 0 });
  Object.assign(players[0], { x: 270, y: H / 2, vx: 0, vy: 0 });
  Object.assign(players[1], { x: 730, y: H / 2, vx: 0, vy: 0 });
}

function setControls() {
  const c = $("controls");
  if (!c) return;
  c.innerHTML = mode === "local"
    ? '<p><span class="dot green-dot"></span><b>Verde:</b> WASD · X para patear</p><p><span class="dot blue-dot"></span><b>Azul:</b> Flechas · M para patear</p>'
    : mode === "practice"
    ? `<p><span class="dot green-dot"></span><b>Vos (Verde):</b> WASD · X para patear</p><p><span class="dot blue-dot"></span><b>Bot Azul:</b> ${botDifficulty === "beginner" ? "Debutante" : botDifficulty === "pro" ? "Professional" : "GOAT"}</p>`
    : mode === "host"
    ? '<p><span class="dot green-dot"></span><b>Vos (Verde):</b> WASD · X para patear</p><p><span class="dot blue-dot"></span>Tu rival también usa WASD · X</p>'
    : '<p><span class="dot blue-dot"></span><b>Vos (Azul):</b> WASD · X para patear</p><p><span class="dot green-dot"></span>Tu rival controla Verde</p>';
}
...
function closeNetwork() {
  ...
  connection = peer = localChannel = null;
}
...
function createMatch() {
  ...
  if (!window.Peer) {
    if ($("lobby-status")) $("lobby-status").textContent = "No se pudo cargar la conexión. Revisá tu internet.";
    return;
  }
  ...
  peer.on("error", () => {
    if ($("lobby-status")) $("lobby-status").textContent = "No se pudo crear la sala. Intentá nuevamente.";
    if ($("create-button")) $("create-button").disabled = false;
  });
}
...
function createLocalConnection(id, host) {
  ...
  return {
    ...
    on: (event, callback) => {
      if (event === "data") localChannel.addEventListener("message", e => callback(e.data));
      if (event === "open" && !host) queueMicrotask(callback);
    },
...
function configureConnection(host) {
  connection.on("open", () => {
    if (host) startGame("host");
    else { mode = "guest"; setControls(); show("match"); message("Conectado. Esperando al anfitrión…"); }
  });
...
function handleKey(e, down) {
  ...
  if (mode === "guest" && connection?.open && ["w", "a", "s", "d", "x"].includes(key)) {
    connection.send({ type: "input", key, down });
  }
  if (mode === "local" && key === "r" && down && !e.repeat) {
    scores = { green: 0, blue: 0 }; updateScore(); resetPositions(); message("¡Marcador reiniciado!");
  }
}
...
function initTouchControls() {
  ...
  if (kickBtn) {
    kickBtn.addEventListener("touchstart", e => {
      e.preventDefault();
      localKeys.add("x");
      if (mode === "guest" && connection?.open) connection.send({ type: "input", key: "x", down: true });
    }, { passive: false });

    const releaseKick = e => {
      e.preventDefault();
      localKeys.delete("x");
      if (mode === "guest" && connection?.open) connection.send({ type: "input", key: "x", down: false });
    };
...
function clamp(body, max) {
  const s = Math.hypot(body.vx, body.vy);
  if (s > max) { body.vx = body.vx / s * max; body.vy = body.vy / s * max; }
}

function updatePlayer(p, dt, input) {
  const [up, down, left, right] = p.controls;
  let ax = (input.has(right) ? 1 : 0) - (input.has(left) ? 1 : 0);
  let ay = (input.has(down) ? 1 : 0) - (input.has(up) ? 1 : 0);
  if (ax || ay) { const d = Math.hypot(ax, ay); ax /= d; ay /= d; }
  p.vx += ax * 1280 * dt;
  p.vy += ay * 1280 * dt;
  p.vx *= Math.pow(.0008, dt);
  p.vy *= Math.pow(.0008, dt);
  clamp(p, p.maxSpeed || 410);
  p.x += p.vx * dt;
  p.y += p.vy * dt;
  p.x = Math.max(field.left + p.r, Math.min(field.right - p.r, p.x));
  p.y = Math.max(field.top + p.r, Math.min(field.bottom - p.r, p.y));
  if (input.has(p.kick) && p.kickReady) kick(p);
  if (!input.has(p.kick)) p.kickReady = true;
}
...
function kick(p) {
  ...
  if (d < p.r + ball.r + 43) {
    ball.vx += dx / d * 510 + p.vx * .28;
    ball.vy += dy / d * 510 + p.vy * .28;
    clamp(ball, 720);
  }
}

function collide(a, b, restitution = .72) {
  let dx = b.x - a.x, dy = b.y - a.y, d = Math.hypot(dx, dy), min = a.r + b.r;
  if (d >= min) return;
  if (!d) { dx = 1; d = 1; }
  const nx = dx / d, ny = dy / d, ia = 1 / a.mass, ib = 1 / b.mass, overlap = min - d;
  ...
  if (rel < 0) {
    const imp = -(1 + restitution) * rel / (ia + ib);
...
function updateBall(dt) {
  ...
  clamp(ball, 780);
  const mouth = ball.y > field.goalTop && ball.y < field.goalBottom;
  if (ball.y - ball.r < field.top) { ball.y = field.top + ball.r; ball.vy = Math.abs(ball.vy) * .83; }
  if (ball.y + ball.r > field.bottom) { ball.y = field.bottom - ball.r; ball.vy = -Math.abs(ball.vy) * .83; }
  if (!mouth && ball.x - ball.r < field.left) { ball.x = field.left + ball.r; ball.vx = Math.abs(ball.vx) * .83; }
  if (!mouth && ball.x + ball.r > field.right) { ball.x = field.right - ball.r; ball.vx = -Math.abs(ball.vx) * .83; }
  if (mouth && ball.x + ball.r < field.left) score("blue");
  if (mouth && ball.x - ball.r > field.right) score("green");
}

function score(team) {
  if (kickoffUntil) return;
  scores[team]++;
  updateScore();
  message(`¡GOL DE ${team === "green" ? "VERDE" : "AZUL"}!`);
  kickoffUntil = performance.now() + 1300;
  setTimeout(() => { resetPositions(); message("¡A jugar!"); kickoffUntil = 0; }, 1300);
}

function updateBot(now) {
  const p = players[1], rival = players[0], profile = botProfiles[botDifficulty], goal = { x: field.left - 25, y: H / 2 };
  if (now >= botThinkAt) {
    const gx = goal.x - ball.x, gy = goal.y - ball.y, gd = Math.hypot(gx, gy) || 1;
    const nearRival = Math.hypot(rival.x - ball.x, rival.y - ball.y) < 120;
    const side = nearRival ? (rival.y < ball.y ? 1 : -1) : Math.sin(now / 360) * .45;
    botTarget = { x: ball.x - gx / gd * profile.offset, y: ball.y - gy / gd * profile.offset * .3 + side * profile.dribble };
    botTarget.x = Math.max(field.left + p.r, Math.min(field.right - p.r, botTarget.x));
    botTarget.y = Math.max(field.top + p.r, Math.min(field.bottom - p.r, botTarget.y));
    botThinkAt = now + profile.reaction;
  }
  remoteKeys.clear();
  if (botTarget.x < p.x - 9) remoteKeys.add("a");
  if (botTarget.x > p.x + 9) remoteKeys.add("d");
  if (botTarget.y < p.y - 9) remoteKeys.add("w");
  if (botTarget.y > p.y + 9) remoteKeys.add("s");
  const close = Math.hypot(ball.x - p.x, ball.y - p.y) < p.r + ball.r + 42;
  const behindBall = p.x > ball.x - 8;
  if (close && behindBall) remoteKeys.add("x");
}

function updateTimerDisplay(ms) {
  try {
    const reloj = $("timer-display");
    if (!reloj) return;
...
function update(dt, now) {
  try {
    ...
    if (players[1]) updatePlayer(players[1], dt, mode === "local" ? localKeys : remoteKeys);
...
function drawField() {
  ...
  ctx.fillStyle = "#2e9a55"; ctx.fillRect(0, 0, W, H);
  ctx.fillStyle = "rgba(255,255,255,.035)";
  for (let x = field.left; x < field.right; x += 100) ctx.fillRect(x, field.top, 50, field.bottom - field.top);
  ...
  line(W / 2, field.top, W / 2, field.bottom);
  ctx.beginPath(); ctx.arc(W / 2, H / 2, 84, 0, Math.PI * 2); ctx.stroke();
  ctx.beginPath(); ctx.arc(W / 2, H / 2, 5, 0, Math.PI * 2); ctx.fillStyle = "white"; ctx.fill();
  ctx.strokeStyle = "rgba(245,250,255,.9)"; ctx.lineWidth = 4;
  ctx.strokeRect(field.left - field.depth, field.goalTop, field.depth, field.goalBottom - field.goalTop);
  ctx.strokeRect(field.right, field.goalTop, field.depth, field.goalBottom - field.goalTop);
}
...
function drawPlayer(p) {
  ...
  ctx.beginPath(); ctx.arc(2, 4, p.r, 0, Math.PI * 2); ctx.fillStyle = "rgba(0,0,0,.18)"; ctx.fill();
  ...
  ctx.beginPath(); ctx.arc(-7, -8, 7, 0, Math.PI * 2); ctx.fillStyle = "rgba(255,255,255,.32)"; ctx.fill();
  ctx.restore();
}
...
function drawBall() {
  ...
  ctx.beginPath(); ctx.arc(2, 3, ball.r, 0, Math.PI * 2); ctx.fillStyle = "rgba(0,0,0,.22)"; ctx.fill();
